...
バージョンポリシー検査を含む WikiBrain クライアントは、HTTPS 経由で
[`release-policy.json`](release-policy.json) を最大 24 時間に 1 回確認します。
インストール済みバージョンが `minimum_supported_version` 未満の場合、取得、
呼び出し、書き込み、再インデックス、resume、実行を伴う初期化を停止し、正確な
Homebrew 更新コマンドを表示します。
...
ネイティブ Windows では、以下で説明するバージョン固定のインストーラー URL を
表示します。ダウンロードした内容を確認してから実行してください。`pipx` で直接
...
フック/スキルの確認とアンインストールは引き続き利用できます。サポート終了版でも、
診断、プライバシー制御、ユーザー所有データへのアクセスは妨げません。ネットワーク、
schema、不正なポリシーのエラーは fail-open とし、失敗結果を 24 時間 negative
cache します。この検査は workspace、prompt、memory の内容を送信しません。

Homebrew は既にインストール済みのバイナリへこの検査を後付けできません。検査機能が
導入される前のリリースは一度手動で更新する必要があり、それ以降のポリシー変更から
最低バージョン enforcement が適用されます。
