...
<p align="center">
  <img src="docs/assets/benchmark-retrieval-quality-v1.svg" width="920" alt="WikiBrain 검색 품질 벤치마크: Recall@1 69.44%, Recall@3 87.50%, nDCG@3 81.35%, MRR 87.50%, Top-1 출처 일치율 83.33%, 문서 14개 중 14개 적재, 금지 문서 노출 0%">
</p>

지연시간과 별도로 정답 label이 있는 corpus로 검색 순위를 측정합니다. 합성 문서
14개를 적재하고 그중 1개를 index 후 삭제한 다음, 정확한 표현·바꿔 말하기·다중
정답·전역 선호·workspace 범위 질의 12개를 실행합니다.
...
| Top-1 출처 일치율 | **83.33%** |
| 금지 문서 노출 | **0/12 query · 0.00%** |

`forbidden` label은 다른 workspace, 폐기된 결정, 삭제된 기억을 포함합니다. 완벽하지
않은 검색 점수는 의도적입니다. 바꿔 말하기와 순위 약점을 드러내며, 기능 검사
8/8을 검색 정확도로 과장하지 않습니다.
...
# /tmp 파일의 documents, queries, relevant, forbidden을 수정합니다.
uv run --locked python -m benchmarks.retrieval_quality \
