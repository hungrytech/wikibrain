...
적응형 승격은 최근 60일 sliding window를 사용합니다. session 또는 handoff가 서로
다른 UTC 날짜 3일 이상, 서로 다른 consumer provider/session pair 3개 이상에서 실제
최종 맥락에 포함되고 provider/session/day 기준 주입이 2회 이상이어야 합니다. 같은
provider/session pair·같은 날의 재호출은 한 번만 셉니다. 실제 consumer session
identity가 없는 수동 `brainctl recall`은 세지 않습니다. 최종 `<memory-data>`에
도달하지 않은 검색 결과는 세지
않고, memory 페이지 자체는 자신의 승격 점수를 높이지 않으며, superseded 근거와
