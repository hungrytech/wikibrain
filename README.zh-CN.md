...
自适应提升使用最近 60 天的滑动窗口。session 或 handoff 必须在至少 3 个不同的
UTC 日期、3 个不同的 consumer provider/session pair 中实际进入最终上下文，并
至少发生 2 次按 provider/session/day 计的注入。同一 provider/session pair 在
同一天的重放只计一次。没有真实 consumer session identity 的手动
`brainctl recall` 不计数。未进入最终 `<memory-data>` 的搜索结果不计数，memory
页面不会增加自身的提升分数，
